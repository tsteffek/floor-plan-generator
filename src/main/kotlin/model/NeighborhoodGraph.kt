...
data class NeighborhoodGraph<T : GeometricObject>(
    ...
    /** Returns the closest neighbors to [T]. Size may vary depending on how the graph was created. */
    fun getNeighbors(t: T): Set<T> = map.getValue(t)

    companion object {
        /**
         * Computes the [NeighborhoodGraph] of a [List] of [T]s using brute force. Depending on the type of [T], more efficient algorithms may be available.
         * @param closestNeighbors amount of closest neighbors to detect, defaults to 2
         */
        ...
        /**
         * Computes a [NeighborhoodGraph] utilizing inherent properties of [PolarPoint]s.
         * @return a graph with exactly 2 neighbors per [PolarPoint], which are the closest neighbors *per side*.
         */
